...
export const useApis = () => {
  const get = (path, isExternal) => {
    const url =
      isExternal == true ? path : "https://swapi.dev/api" + (path ?? "/");
  ...
  const setImagesFromDB = (path, original, key, keyName) => {
    if (key != null) {
      return Object.assign(original, {
        image:
          "https://raw.githubusercontent.com/DiachenkoRoman/SW_Database_prj/master/img/" +
          path +
          "/" +
          key.split(" ").join("") +
          ".jpg",
        ...
        item.image == undefined
          ? Object.assign(item, {
              image:
                ...
                path +
                "/" +
                (item[keyName] ?? item.name).split(" ").join("") +
                ".jpg",
            })
          : item
      );
